...
)
{
    ...
    {
	auto path_fragment = separate_fragment(navpoint.src_absolute);
	auto path = path_fragment.first;
	auto fragment = path_fragment.second;
...
)
{
    for (uint32_t spine_idx = 0; spine_idx < package.spine_ids.size(); ++spine_idx)
    {
        ...
        if (item_it != package.id_to_manifest_item.end())
        {
            ...
            {
                out_toc.emplace_back(TocItemCache {
                    std::experimental::filesystem::path(item_it->second.href).filename(),  // fallback name
                    0,
                    spine_idx,
                    ""
                });
...
std::experimental::optional<uint32_t> EpubTocIndex::get_toc_item_index(const DocAddr &address) const
{
    const auto &toc = state->toc;
