...
std::experimental::filesystem::path normalize_path(const std::experimental::filesystem::path& p) {
    std::experimental::filesystem::path result;
    for (const auto& component : p) {
        if (component == ".") {
            continue;  // Skip current directory
        } else if (component == "..") {
            result = result.parent_path();  // Go up one level
        } else {
            result /= component;
        }
    }
...
std::unordered_map<std::string, ManifestItem> parse_package_manifest(const std::experimental::filesystem::path &base_path, xmlNodePtr node)
{
...
bool epub_parse_package_contents(const std::string &rootfile_path, const char *package_xml, PackageContents &out_package)
{
    ...
    std::experimental::filesystem::path base_path = std::experimental::filesystem::path(rootfile_path).parent_path();
...
void parse_nav_point(const std::experimental::filesystem::path &base_path, xmlNodePtr node, std::vector<NavPoint> &out)
{
...
void parse_nav_map(const std::experimental::filesystem::path &base_path, xmlNodePtr node, std::vector<NavPoint> &out)
{
...
bool epub_parse_ncx(const std::string &ncx_file_path, const char *ncx_xml, std::vector<NavPoint> &out_navmap)
{
    ...
    std::experimental::filesystem::path base_path = std::experimental::filesystem::path(ncx_file_path).parent_path();
...
std::experimental::optional<NavPoint> try_parse_anchor(xmlNodePtr anchor, const std::experimental::filesystem::path &base_path)
{
    if (anchor)
    {
        ...
        if (href && xmlStrlen(href))
        {
            ...
            if (!label.empty())
            {
                return NavPoint(
                    label,
                    (const char*)href,
                    normalize_path(base_path / ((const char*)href))
                );
            }
        }
    }
...
std::experimental::optional<NavPoint> try_parse_span(xmlNodePtr span)
{
    if (span)
    {
...
void parse_ol(xmlNodePtr node, const std::experimental::filesystem::path &base_path, std::vector<NavPoint> &out);

void parse_li(xmlNodePtr li_node, const std::experimental::filesystem::path &base_path, std::vector<NavPoint> &out)
{
...
void parse_ol(xmlNodePtr ol_node, const std::experimental::filesystem::path &base_path, std::vector<NavPoint> &out)
{
...
bool epub_parse_nav(const std::string &nav_file_path, const char *nav_xml, std::vector<NavPoint> &out_navmap)
{
    ...
    std::experimental::filesystem::path base_path = std::experimental::filesystem::path(nav_file_path).parent_path();
