...
{
public:
    virtual std::experimental::optional<std::string> read(const std::string &book_id, const std::string &key) const = 0;
...
public:
    ...
    virtual std::vector<char> load_resource(const std::experimental::filesystem::path &path) const = 0;
};
